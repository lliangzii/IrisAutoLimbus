...
def click_img(window,image_path):
...
def auto_fighting(window):

    while True:
        if find_image(window,'./img/startTurn_yet.png') is not None:
            click_img(window,'./img/P.png')
            sleep(1)
        if find_image(window,'./img/startTurn.png') is not None:
            click_img(window,'./img/startTurn.png')
            sleep(1)
        if find_image(window,'./img/wait_fighting.png') is not None:
            sleep(30)
        if find_image(window,'./img/dante_inMirror.png') is not None:
            return